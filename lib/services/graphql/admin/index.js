		...
		const resolvers = {
			...
			Mutation: {
				...
				addValue: (obj, args, ctxt) => new Promise((resolve, reject) => {
					const { database, name, primitive } = args;
					...
					// TODO: add validation for database
					// TODO: increment patch of database version
					// TODO: add access control based on the user context
					services.act({ plugin: 'store', cmd: 'add', params }, err => {
						if (err) {
							reject(err);
				...
				addEntity: (obj, args, ctxt) => new Promise((resolve, reject) => {
					if (!ctxt) {
						return reject(new Error('No context was provided'));
					}
					return resolve(RESULT_OK);
				}),
